...
| Hard days in a row | After 2 moderate/high-intensity days in a row → **Go easy** | "Athletes will perform a maximum of two consecutive sessions of moderate or high intensity": [Carrasco-Poyatos et al., 2020](https://pmc.ncbi.nlm.nih.gov/articles/PMC7432021/) — **a published trial protocol, not a result** |
...
| After training | Readiness is an overnight measurement, so a session cannot change this morning's number — it lowers the next one. Measured on your own history: the next night's own composite regressed on today's composite score and today's strain (Newey-West errors, lag 7, because days are dependent). Here the effect is real — **−0.041 SD per strain point, p < 0.001, 526 day pairs**, and monotone across strain terciles (+0.07 / −0.12 / −0.29). **The adjusted score is not shown**, because it fails its own out-of-sample test: fitted on the first 70% of days it predicted the last 30% of next-morning scores *worse* than doing nothing (mean error 13.3 vs 11.4). The display turns itself on only if that reverses | Our own measurement, labelled as an extension — no published model prescribes this |
