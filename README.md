...
| What moves your readiness | **Five** things about a day — bedtime against your own median, day strain, whether you napped, when the last session finished, days since a rest day — each fitted **on its own** against the next morning's standardised score, controlling for that day's score, with Newey-West errors (lag 7). Each must pass **the same three gates** as the training-cost adjustment: significant at p < 0.05; even across its own terciles, judged on the next morning **with today's score taken out** (the outcome the fit actually explains); and, fitted on the first 70% of days, better than doing nothing on the last 30%. Survivors are then **refitted together** in one model: fitted one at a time, correlated candidates borrow each other's effect, so any that **flips sign** jointly is dropped, one that keeps its sign but loses significance says so, and one that is an exact linear combination of candidates already in (entered in the order above) is dropped as redundant. The quoted effect is still the one-at-a-time number; the joint fit is a gate. The candidates overlap, so their effects **do not add up**. **Hours asleep is not a candidate**: it is one of the score's own inputs (today's last-night score) and sits inside tomorrow's 7-day window, so regressing tomorrow on it while controlling for today measures the formula's weights, not sleep — on this history it came out as "an extra hour of sleep costs 2 points", which is mechanical. On this history **one of five** survives: a point of day strain is worth **−1 point** on the next morning, over 527 days, and it improved the held-out forecast by **0.01 points**. Points are read off the normal curve **around a middling day**, where it is steepest — the same shift is worth fewer points at either extreme, so these are the largest the effects can be. **The gates reduce false findings; they do not eliminate them**: on the synthetic athlete, bedtime — which the generator gives no effect at all — passes every gate including the joint one (it fails on five other random seeds; about half of it is strain showing through) | Our own measurement, labelled as one. Gates and errors as in `build_training_cost`; nothing here is a published rule, and nothing acts on the plan |
