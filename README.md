...
| **Bedtime target** | "Asleep by HH:MM" = your usual wake time minus tonight's sleep need. The need is **WHOOP's own**, straight from the API (`sleep_needed` = baseline + sleep debt + recent strain + recent naps) — the only arithmetic here is the subtraction. Usual wake time is the median of the last 30 nights, each in its own time zone. How much your wake time varies is **reported, not judged**: no study defines when a wake time is too variable to aim at, so the card states the spread (1h 30m here) and calls the target rough rather than applying an invented cut-off | WHOOP's `sleep_needed`; median and spread are plain descriptive statistics |
