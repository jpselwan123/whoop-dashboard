...
- **Does following the plan pay off?** — every past day grouped by whether you followed the plan, next to how you
...
| On a readable scale | Averaging standard scores shrinks their spread — the measures move together, but not exactly — so the average is **not** on a 1-SD-per-unit scale of its own (measured here: 0.88 SD over 529 days, which made a genuinely above-normal day read as average). The average is standardised against the spread of your own earlier scores and shown as a percentile of them | Standardising a composite before applying SD cut-offs: Thornton 2019. The percentile itself is the normal curve (Φ), not a choice — no study prescribes a display scale, so nothing here is invented: the lines stay exactly the trials' SD lines |
| Today's plan | **31+** — at or above the bottom of the normal band → **Train as planned** (the trials' prescribed moderate/high session); below the band → **Go easy** or **Rest**. There is no separate answer for being *above* the band, because no cited trial prescribes one: Kiviniemi 2007 gives high intensity on "an increase **or no change**", Javaloyes 2019 on "above **or within** the SWC", Vesterinen 2016 programmes the moderate/high session when HRV is **within** it, and the review of the trials says high intensity "within or above baseline ranges". The band edges are the same ±0.5 SD and −1.5 SD lines read on the percentile scale. Over 529 days they caught 31.4% / 37.4% / 25.3% / 5.9% of days, against the 30.9 / 38.3 / 24.2 / 6.7% the SD lines correspond to — that is a check that **the scale is built correctly**, not evidence that the score predicts anything, since a percentile puts a known fraction either side of each line by construction | ±0.5 SD decision line: Javaloyes 2019, [Kiviniemi et al., 2007](https://pubmed.ncbi.nlm.nih.gov/17849143/); 1.5 SD below "worth acting on": Thornton 2019 |
| Go easy or rest | Below the band the trials prescribe "low intensity exercise (or passive rest)". Rest when the fall is **large** (under 7, i.e. 1.5 SD below) or **sustained** — the **third day in a row** below your normal, because a run of low days, not one low night, is what these measures are read on. Never more than **2 rest days in a row**, the limit HRV-guided protocols put on consecutive rest days. Here that gives a rest day about once a week (14% of days) instead of once every 18 | "Low intensity exercise (or passive rest) … when values are suppressed": [Manresa-Rocamora et al., 2021](https://pmc.ncbi.nlm.nih.gov/articles/PMC8507742/); sustained rather than single-day changes: [Plews et al., 2013](https://pubmed.ncbi.nlm.nih.gov/23539308/), [Buchheit, 2014](https://www.frontiersin.org/journals/physiology/articles/10.3389/fphys.2014.00073/full); cap on consecutive rest days: Kiviniemi 2007 |
...
| Through the day | Every refresh recomputes the load ratio with today's strain so far. Before a session, the plan steps down as it climbs: past **1.5** it becomes Go easy. The card says how much strain is left before 1.3 ("passes 1.3 if today's strain goes over 14.2") — the day strain that puts the ratio exactly on the line, solved from the same 7- and 28-day averages. A nap changes readiness itself, because hours asleep count naps. "Today" is the current WHOOP day, from one sleep to the next | Load-ratio bands: [Gabbett, 2016](https://bjsm.bmj.com/content/50/5/273) — disputed as an injury predictor: [Impellizzeri et al., 2020](https://pubmed.ncbi.nlm.nih.gov/32502973/) |
| After training | Readiness is measured overnight, so a session can't change this morning's number — but it lowers the next one. On your own history, each past day's strain is set against the next morning's readiness, holding that day's readiness constant (least squares; used only if the effect is negative and significant, p < 0.05, 30+ pairs). Once you've trained, the orb shows this morning's score minus that cost for today's strain above a typical rest day (median rest-day strain) — never a bonus for doing less. Here: 0.8 points per strain point, 521 pairs; 33 this morning with a 16.0 day → 24 | Own-data measurement, same conventions as every comparison on the page |
...
Last night is **half the score** — three of the six inputs — and the trend over the previous 7 days is
the other half. Because the trend window stops the day before, each night is counted once; when it was
also inside the trend average, last night carried about 57% of the score and the plan changed on 41% of
days instead of 27%, churn that came from the double count rather than from the body. The score needs
...
| **Does following the plan pay off?** | Next-morning recovery after days the plan was followed, against days a moderate or high-intensity session was done on a **Go easy** or **Rest** day. Welch's t-test, p < 0.05, 30+ per group; with fewer the counts are shown and nothing is claimed. Comparing the score with recovery was dropped: recovery is built from HRV and resting heart rate, the same measures the score is built from, so the two agree by construction. **The p-value here is optimistic** — consecutive days are not independent (the score's day-to-day correlation is about 0.87), so a real difference is easier to "find" than the test assumes | Standard statistical conventions, with the dependence caveat stated |
...
own middle. The band shares landing within about a point of their intended values shows that holds —
but it is a property of the scale, not evidence the score is right about anything. Zone
