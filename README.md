...
| Breathing rate | Last night against your usual (average of nights 30–90 days before, 30+ nights), **reported and never acted on**. The old "3+ breaths/min → Rest" rule was ours, not Natarajan's — the paper scores a standardised deviation from a rolling baseline and gives no numeric rise — and over 529 nights it never once fired (largest rise +2.3/min) | [Natarajan et al., 2021](https://pmc.ncbi.nlm.nih.gov/articles/PMC8443549/) |
| Hard days in a row | After 2 moderate/high-intensity days in a row → **Go easy** | Carrasco-Poyatos 2020 |
| Through the day | Every refresh recomputes the load ratio with today's **training load** (Edwards TRIMP) so far. Before a session, the plan steps down as it climbs: past **1.5** it becomes Go easy. The card says how much room is left before 1.3 ("passes 1.3 if today's training load goes over 340") — the load that puts the ratio exactly on the line, solved from the same 7- and 28-day averages. A nap changes readiness itself, because hours asleep count naps. "Today" is the current WHOOP day, from one sleep to the next | Load-ratio bands: [Gabbett, 2016](https://bjsm.bmj.com/content/50/5/273) — disputed as an injury predictor: [Impellizzeri et al., 2020](https://pubmed.ncbi.nlm.nih.gov/32502973/) |
| After training | Readiness is an overnight measurement, so a session cannot change this morning's number — it lowers the next one. Measured on your own history: the next night's own composite regressed on today's composite score and today's strain (Newey-West errors, lag 7, because days are dependent). Here the effect is real — **−0.042 SD per strain point, p < 0.001, 526 day pairs**, and monotone across strain terciles (+0.07 / −0.13 / −0.29). **The adjusted score is not shown**, because it fails its own out-of-sample test: fitted on the first 70% of days it predicted the last 30% of next-morning scores *worse* than doing nothing (mean error 13.2 vs 11.3). The display turns itself on only if that reverses | Our own measurement, labelled as an extension — no published model prescribes this |
