...
def _h(ms):
    return "" if ms is None else _n(ms / 3600000, 2)
...
def build_context_text():
    ...
    for s in sorted(scored(raw["sleep"]), key=lambda s: s["start"]):
        ...
        sleeps.append([
            ...
            _h(st.get("total_in_bed_time_milli")), _h(st.get("total_awake_time_milli")),
            _h(st.get("total_light_sleep_time_milli")), _h(st.get("total_slow_wave_sleep_time_milli")),
            _h(st.get("total_rem_sleep_time_milli")), _n(st.get("sleep_cycle_count"), 0),
            _n(st.get("disturbance_count"), 0), _n(sc.get("respiratory_rate")),
            _n(sc.get("sleep_performance_percentage"), 0), _n(sc.get("sleep_consistency_percentage"), 0),
    ...
    parts = [
        ...
        _table("days", "One row per WHOOP day (cycle). strain 0–21; kcal = total energy; "
               ...
               days),
        _table("sleeps", "One row per sleep. Times are local. Hours unless noted; resp = breaths/min; "
               "perf/consistency/efficiency are %; need_* = hours of sleep WHOOP says were needed from baseline, "
               "sleep debt, recent strain, and recent naps.",
               ["date", "type", "start", "end", "in_bed_h", "awake_h", "light_h", "deep_h", "rem_h", "cycles",
                "disturbances", "resp", "perf", "consistency", "efficiency",
                "need_baseline_h", "need_debt_h", "need_strain_h", "need_nap_h"],
               sleeps),
